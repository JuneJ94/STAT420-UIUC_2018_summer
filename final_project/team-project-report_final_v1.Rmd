...
The `train` subset contains 4850 observations and the `test` dataset contains the rest. 
...
#### 3.2 Start with a simple additive model with all posibile attributes (Set $\alpha = 0.05$):
...
get_num_params(bic)
```
Both models seem to have pretty high VIF, which suggests multicollinearity. Next, we try to find a new pair of models with smaller value of VIF. 

#### 3.5 Build models without multicollinearity:

Find significant variables from the intercation model. Set $\alpha = 0.05$
```{r}
alpha = 0.05
...
length(variables)
```
From the above code, we can see that among `r length(variables)` variables, variables `attr_o`, `shar_o`, `gender1`, `dec_o1` and the intercation items,  `age_o:shar_o`, `attr_o:samerace1`, `shar_o:dec_o1` and `gender1:dec_o1` are significant. 

Start with the intercation model with all significant variables listed above and find a new pair of `aic_new` and `bic_new` model. 
```{r}
...
##### 3.7.1 Check values of `LOOCV_RMSE`, adjusted $R^2$, number of $\beta$ and maximum VIF
...
From the table above, we could tell the new pair of aic and bic models have much smaller maximum VIF and much less predictors than the old pair. And their values of `LOOVC_RMSE` and adjusted $R^2$ do not change a lot, their values of `LOOCV_RMSE` raise a little bit and values of adjusted $R^2$ drops a little bit compared to the old pair which is acceptable. We choose these two new models to be our selected models. 
...
From the "Fitted vs Residuals" plot and "Q-Q" plot, both two selected models look pretty good. But from the results of "Shapiro-Wilk normality test" and "Breusch-Pagan test", the p-values are pretty small, which means both constant variance and normality assumption have been violated. We are not surprised to see this violation due to the data distribution. 

##### 3.7.3 Model comparison between aic and bic
...
From the table above, `bic_new` model has 7 predictors less than `aic_new` model while its value of `LOOCV_RMSE` is only a little bit higher than the `aic_new` model and its adjusted $R^2$ of is only a little bit smaller. With small difference in `LOOCV_RMSE` and  adjusted $R^2$ , we're looking for a smaller and easier-to-interpret model, therefore we come to the conclusion that `bic_new` model is better than `aic_new` model. 
...
Based on the table above, `bic_new` model has 4 predictors less than the simple additive model, smaller value of `LOOCV_RMSE` and higher value of adjusted $R^2$. Therefore we will arrive at a conclusion that `bic_new` model is better than `simple_model` model (additive model). 

**In conclusion, the "best" model we find in this project would be `bic_new` model. Next, we will go throught some evaluations on this model.**
...
- `dec_o` is the dummy variable, $\text{dec_o} = 1$ when making the decision to see partner again, $\text{dex_o} = 0$ when making the decision not to see partner again. 
...
Since we have one dummy variable in the model, `dec_o`, we would like to consider two different scenarios: $\text{dec_o} = 0$ when the participant makes a decision not to see partner again. And $\text{dec_o} = 1$ when the participant makes a decision to see partner again.
...
From the plot above, we can see that how people rate their partners on attrativeness and shared-interest have the strongest correlation on their rating of how much they like their partners **when they decide not to see them again**; fun and sincerity follow and intelligence has the least correlation in the model.  
...
From the plot above, we can see that **when people decide to see their partners again**, the correlation between how they rate their partners on like and rate on shared-interest drops a little; attrativeness remains the strongest correlation and the rest of variables stay the same.  
...
- Does intellingence correlate with how people like their partners?

According to our model, yes, intellingence has some correlation.
...
According to our model, yes, different decision have difference influence on how shared-interests relates with how people like their partners.  
...
Since our reponse has non-positive value 0 (only 5 observations), we need to remove them before try box-cox transformation.
...
We're using backward AIC and BIC again, starting with a model with quadratic terms for all posibile variables. BIC selects a model with 2 predictors less than AIC and all quadratic terms are **not significant**. 

### 5. Will the result change if the starting model has less attributes? 

Based on the previous section, the additive model with all variables has the smallest maxmimum VIF among all the models, which is `r max(vif(simple_model))`. We will discuss if result changes when model selection starts with an additive model containing six attributes.  
```{r}
...
Next, make a table to list maximum VIF, number of $\beta$, `LOOCV_RMSE` and adjusted $R^2$ for comparison with `aic_test` model and `bic_new` model. 
...
From the table above, we could tell that even though `bic_new` model has a smaller maximum VIF and number of $\beta$, but it **hasn't improved on `LOOCV_RMSE` or adjusted $R^2$** compared with `bic_model`. 
...
Compare with maximum VIF and number of predictors. 
...
```
From the above code, we could tell that by removing the influential observations, the aic model gets two predcitors less and bic model's number of predictors remains the same.

Compare with `LOOCV_RMSE`, numbers of $\beta$, maximum VIF and adjusted $R ^ 2$ of two paris of selected models
```{r}
...
From the table above, for both `aic_new` model and `bic_new` model, their `LOOCV_RMSE` decreases and adjusted $R^2$ increases which are expected by removing the influential observations. However, **both models' max VIF increase which is considered as a drawback**.

Next, we will evaluate the two new subset models by calculating the average precentage error and compare the values of average precentage error without removing influential observations. 
...
**When apply the models to test data, we don't see error rate drop dramatically. Besides, the quantity of influential observations is less than 10% of total quantity of `train_data` subset observations, therefore we decide not to remove any outlier at this stage.**
...
hat_values_bic
```
- Observations identified influential
```{r}
###in aic_new model
cooks_dis_aic
```
